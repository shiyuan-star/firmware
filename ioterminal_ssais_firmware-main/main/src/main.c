...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(led_indicator_start(digitalOutputHandle[0], DIGITAL_OUTPUT_SLOW_BLINK_LOOP));
    ESP_ERROR_CHECK(led_indicator_start(digitalOutputHandle[1], DIGITAL_OUTPUT_SLOW_BLINK_LOOP));
    ESP_ERROR_CHECK(led_indicator_start(sysStateLedHandle, STATE_LED_BREATHE));            // 默认状态灯效
    ESP_ERROR_CHECK(led_indicator_preempt_start(sysStateLedHandle, STATE_LED_BOOT_BLINK)); // 抢占启动闪烁
    ...
    // g_dioInpDataQueueHandler = xQueueCreate(DIO_RECEIVE_QUEUE_LEN, sizeof(DioInputData_t));
    ...
    if (g_nvsData.DeviceConfigData.ledstripConfigData.ledstripEnabled)
    {
        ...
        led_strip_clear(g_ledstripRmtHandle); // 带电复位的情况下，清除残留的灯珠
    ...
    while (getNetTaskState() != NET_TASK_READY) // 等待设备网络准备完毕
    {
