...
// MQTT CONFIG
#define MQTT_RECEIVE_QUEUE_LEN 16 // Mqtt 接收数据队列长度
#define MQTT_PUBISH_QUEUE_LEN 16  // Mqtt 发送数据队列长度
#define MQTT_TOPIC_MAX_LEN 128
#define MQTT_RECEIVE_DATA_MAX_LEN 4096
...
typedef enum
{
    ...
    ALARM_STATE_NET_DISCONNECT,
    ALARM_STATE_NET_CONNECT,
    ALARM_STATE_MQTT_DISCONNECT,
    ALARM_STATE_MQTT_CONNECT,
...
{
    bool otaEnabled;
    char esp32OtaServer1[MAX_URL_BUF_LEN];
    char esp32OtaServer2[MAX_URL_BUF_LEN];
    char esp32OtaServer3[MAX_URL_BUF_LEN];
} OtaConfigData_t;
...
typedef struct _SysStateInfo
{
    ...
    uint32_t bootFreeHeapSize;          // 系统启动后的堆内存
