...
esp_err_t otaSetHandle(uint16_t controlId, uint8_t param[256], uint16_t size)
{
    ...
    switch (controlId)
    {
    case SCREEN_OTA_SERVER_URL_TEXT:
        strcpy(otaConfigData.esp32OtaServer, (char *)param);
        break;
    case SCREEN_OTA_FIRMWARE_FILE_NAME_TEXT:
        strcpy(otaConfigData.firmwareFileName, (char *)param);
        break;
    case SCREEN_OTA_START_OTA_BUTTON:
        if (!param[1]) // 按键松开不处理，按下时触发升级命令
        {
            break;
        }
        // 配置有变更,先存储后更新
